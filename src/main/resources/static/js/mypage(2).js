...
function handleTabChange(e) {
    ...
    // ⭐ 수정: 클릭한 요소를 active로 표시
    this.classList.add('active');

    // ⭐ 수정: 올바른 탭 콘텐츠 찾기 (ID로 검색)
    const selectedContent = document.querySelector(`#${tabName}-tab`);
    if (selectedContent) {
        selectedContent.classList.add('active');
        console.log(`✅ 탭 변경 완료: ${tabName}`);
    } else {
    ...
    fetch(`${API_BASE}/profile/image`, {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                document.querySelector('.profile-pic').src = '/img/default-profile.png';
...
function handleCheckNickname() {
    ...
    if (!nickname) {
        showToast('닉네임을 입력하세요', 'warning');
        return;
    }

    fetch(`${API_BASE}/profile/check-nickname?nickname=${encodeURIComponent(nickname)}`)
        .then(res => res.json())
        .then(data => {
...
function handleBasicInfoSubmit(e) {
    ...
    const formData = {
        ...
        gradeYear: document.getElementById('grade-year').value.trim(),
    ...
    fetch(`${API_BASE}/profile/comprehensive`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
        },
