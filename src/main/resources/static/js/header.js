...
function setupProfileDropdown() {
  ...
  const dropdownMenu = document.querySelector(".sl-dropdown-menu");
  ...
  if (dropdownMenu) {
    document.addEventListener("click", function (e) {
      if (!e.target.closest(".sl-dropdown")) {
        dropdownMenu.classList.remove("show");
        if (e.target.closest(".sl-dropdown-toggle")) {
          return;
        }
      }
    });
  ...
  try {
    ...
    ddaySpans.forEach((span, index) => {
      ...
      if (!originalText || originalText === "" || isNaN(originalText)) {
        ...
        const ddayValue =
          span.getAttribute("data-dday") ||
          span.parentElement.getAttribute("data-dday");
...
function setupMenuEvents() {
  ...
  const dropdownItems = document.querySelectorAll(".sl-dropdown-item");
...
function setupMobileMenuAutoClose() {
  const navLinks = document.querySelectorAll(
    ".header-nav-link, .sl-dropdown-item"
  );
  navLinks.forEach((link) => {
    link.addEventListener("click", function () {
